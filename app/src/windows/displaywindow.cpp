...
DisplayWindow::DisplayWindow()
    : RouteAwareWindow(), _ncTitleWin(nullptr), _ncMenuWin(nullptr), _ncSubMenuWin(nullptr), _ncPanelWin(nullptr),
      _ncMenu(nullptr), _toggle(false)
{
}
...
void DisplayWindow::enterPressed()
{
    ...
    if (item != nullptr)
    {
        ...
        if (Database::instance()->getNode(_route)->children[index].isGroup())
        {
            _toggle = false;
...
void DisplayWindow::updatePanel()
{
    ...
    if (item != nullptr)
    {
        ...
        if (!Database::instance()->getNode(_route)->children[index].isGroup())
        {
            std::string name = Database::instance()->getNode(_route)->children[index].name;
            wattron(_ncPanelWin, A_BOLD);
            mvwprintw(_ncPanelWin, 0, 0, "NAME:");
            wattroff(_ncPanelWin, A_BOLD);
            mvwprintw(_ncPanelWin, 1, 0, name.c_str());

            std::string identity = Database::instance()->getNode(_route)->children[index].identity;
            wattron(_ncPanelWin, A_BOLD);
            ...
            if (_toggle)
            {
                std::string password = Database::instance()->getNode(_route)->children[index].password;
            ...
            std::string more = Database::instance()->getNode(_route)->children[index].more;
            wattron(_ncPanelWin, A_BOLD);
...
WindowAction DisplayWindow::onKeyEvent(int ch)
{
    ...
    if (std::string(keyname(ch)) == "^E")
    {
        spdlog::get("filelogger")->debug("selectedRoute: {}", selectedRoute());
        if (selectedRoute() != "")
        {
    ...
    if (std::string(keyname(ch)) == "^I")
        copyIdentityToClipboard();

    if (std::string(keyname(ch)) == "^P")
...
void DisplayWindow::update()
{
    ...
    int nbShortcuts = 4;
    mvprintw(rows - 4, 0, "Ctrl+N: new");
    mvprintw(rows - 3, 0, "Ctrl+E: edit");
    mvprintw(rows - 2, 0, "Ctrl+L: move");
    mvprintw(rows - 1, 0, "Ctrl+R: remove");
    mvprintw(rows - 4, cols / 3, "Ctrl+I: copy identity");
    mvprintw(rows - 3, cols / 3, "Ctrl+P: copy password");
